...
| `ShortLinkResolutionError` | `SHORT_LINK_NO_LOCATION`, `SHORT_LINK_UNEXPECTED_RESPONSE`, `SHORT_LINK_CHAIN`, `SHORT_LINK_UNSUPPORTED_RUNTIME` | The shortlink server answered in a shape headless cannot extract a target from, or the target is another shortlink. `SHORT_LINK_UNSUPPORTED_RUNTIME` is raised before any HTTP call under Emscripten/Pyodide, where the browser fetch stack follows the redirect the resolver has to read; paste the full report URL instead. CLI exit 1. |
